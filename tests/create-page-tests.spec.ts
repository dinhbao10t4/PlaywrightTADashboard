...
test.describe("Create New Page Tests", () => {
    ...
    test("Verify that Public pages can be visible and accessed by all users of working repository", async ({loginPage, dashboardPage, newPage}) => {
        let mainPage = new MainPage(Utils.generateRandomString(), Constants.EMPTY_STRING, Constants.EMPTY_STRING, Constants.EMPTY_STRING, true);
        ...
        loginPage.login(configuration.sampleRepository, users.johnUser.username, users.johnUser.password);
        await dashboardPage.verifyMainPageDisplays(mainPage.getName());
